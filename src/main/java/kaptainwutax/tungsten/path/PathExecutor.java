...
public class PathExecutor {
    ...
    public void tick(ClientPlayerEntity player, GameOptions options) {
    	...
    	if(this.tick == this.path.size()) {
		    ...
		    if(node.input != null) {
			    ...
			    if (node.input.hasBlockAction()) {
			    	executeBlockAction(player, node.input.action);
			    }
		    }
	...
	private static void executeBlockAction(ClientPlayerEntity player, BlockAction action) {
		MinecraftClient mc = MinecraftClient.getInstance();
		if (mc.interactionManager == null || mc.world == null) return;

		try {
			if (action.hotbarSlot >= 0 && action.hotbarSlot < 9) {
				player.getInventory().setSelectedSlot(action.hotbarSlot);
			}

			if (action.type == BlockAction.Type.BREAK) {
				mc.interactionManager.attackBlock(action.pos, action.face);

			} else if (action.type == BlockAction.Type.PLACE) {
				// anchorPos = pos.offset(face.getOpposite())
				BlockPos anchorPos = action.pos.offset(action.face.getOpposite());
				Vec3d hitVec = Vec3d.ofCenter(anchorPos).add(
					Vec3d.of(action.face.getVector()).multiply(0.5)
				);
				BlockHitResult hitResult = new BlockHitResult(hitVec, action.face, anchorPos, false);
