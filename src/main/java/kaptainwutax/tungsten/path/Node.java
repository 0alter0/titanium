...
import kaptainwutax.tungsten.path.specialMoves.BreakBlockMove;
import kaptainwutax.tungsten.path.specialMoves.PlaceBlockMove;
...
//	    }
	...
	public List<Node> getChildren(WorldView world, Vec3d target, BlockNode nextBlockNode, boolean forceGenAllNodes) {
	    ...
	    if (nextBlockNode.needsBreak()) {
	    	nodes.add(BreakBlockMove.generateMove(
	    		this, nextBlockNode,
	    		nextBlockNode.blockToBreak,
	    		nextBlockNode.breakTicksNeeded,
	    		nextBlockNode.breakToolSlot
	    	));
	    	return nodes;
	    }
	    if (nextBlockNode.needsPlace()) {
	    	Node placeNode = PlaceBlockMove.generateMove(
	    		this, nextBlockNode,
	    		nextBlockNode.blockToPlace,
	    		nextBlockNode.placeToolSlot
	    	);
	    	if (placeNode != this) { nodes.add(placeNode); return nodes; }
	    }
