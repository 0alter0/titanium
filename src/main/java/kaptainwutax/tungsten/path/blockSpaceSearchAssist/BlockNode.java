...
import kaptainwutax.tungsten.helpers.InventorySnapshot;
...
public class BlockNode {
	...
	public BlockPos blockToBreak = null;
	public int breakToolSlot = -1;
	public int breakTicksNeeded = 0;
	public BlockPos blockToPlace = null;
	public int placeToolSlot = -1;
	...
	private boolean shouldRemoveNode(WorldView world, BlockNode child) {
		...
		if (childBelowState.isAir() && !(childBlock instanceof LadderBlock) && !BlockStateChecker.isBottomSlab(childState)) {
			if (BlockShapeChecker.getShapeVolume(child.getBlockPos(), world) == 0) {
				if (tryMarkPlace(child, child.getBlockPos().down(), world)) {
					child.cost += 300; 
				} else {
					return true;
				}
		...
		double heightDiff = getJumpHeight(this.getPos(true).y, child.getPos(true).y); 
...
//                && getShapeVolume(childState.getCollisionShape(world, child.getBlockPos())) >= 1
		...
		if (!wasCleared(world, getBlockPos(), child.getBlockPos(), this, child)) {
			BlockPos obstruction = findObstruction(world, getBlockPos(), child.getBlockPos());
			if (obstruction != null && tryMarkBreak(child, obstruction, world)) {
				child.cost += 200 + child.breakTicksNeeded * 1.5;
			} else {
				return true;
			}
		}
...
//    	}
		...
		double blockHeightDiff = currentBlockHeight - childBlockHeight; 
	...
	private static BlockPos findObstruction(WorldView world, BlockPos from, BlockPos to) {
		int dx = Integer.signum(to.getX() - from.getX());
		int dy = Integer.signum(to.getY() - from.getY());
		int dz = Integer.signum(to.getZ() - from.getZ());

		for (int step = 1; step <= 3; step++) {
			int bx = from.getX() + dx * step;
			int by = from.getY() + dy * step;
			int bz = from.getZ() + dz * step;

			for (int yOff = 0; yOff <= 1; yOff++) {
				BlockPos candidate = new BlockPos(bx, by + yOff, bz);
				BlockState state = world.getBlockState(candidate);
				if (!state.isAir() && BlockShapeChecker.isBlockNormalCube(state)) {
					return candidate;
				}
			}
		}
	...
	private boolean tryMarkBreak(BlockNode child, BlockPos obstruction, WorldView world) {
		BlockState state = world.getBlockState(obstruction);
		if (!AutoToolHelper.isBreakable(state, obstruction, world)) return false;

		AutoToolHelper.ToolResult result = InventorySnapshot.getBestTool(state, obstruction, world);
		if (result.ticksNeeded() == Integer.MAX_VALUE) return false;

		child.blockToBreak = obstruction;
		child.breakToolSlot = result.slot();
		child.breakTicksNeeded = result.ticksNeeded();
		return true;
	}

	private boolean tryMarkPlace(BlockNode child, BlockPos floorPos, WorldView world) {
		int slot = InventorySnapshot.findPlaceableBlock();
		if (slot < 0) return false;

		child.blockToPlace = floorPos;
		child.placeToolSlot = slot;
		return true;
	}

	public boolean needsBreak() { return blockToBreak != null; }

	public boolean needsPlace() { return blockToPlace != null; }
